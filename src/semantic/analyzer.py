...
from src.frontend.ast import (
    ...
    DataStatement, ArithmeticIfStatement, ImplicitNone, ImplicitStatement,
...
class SemanticAnalyzer:
    ...
    def format_error(self, message: str, node: Optional[ASTNode] = None,
    ...
    def format_warning(self, message: str, node: Optional[ASTNode] = None,
    ...
    def analyze(self, ast: Program) -> bool:
        try:
            ...
            self.analyze_declarations(ast.declarations)
    ...
    def get_implicit_type(self, name: str) -> Tuple[TypeKind, Optional[int]]:
        if not name:
    ...
    def analyze_declarations(self, declarations: List[ASTNode]):
        for decl in declarations:
            if isinstance(decl, ImplicitNone):
                ...
                for rule in decl.rules:
                    ...
                    for letter in letters:
                        if letter in self.implicit_rules:
                            ...
                            warning_msg = self.format_warning(
                ...
                for name, dims in decl.names:
                    resolved_dims = self.resolve_dimension_ranges(dims, decl, name) if dims is not None else None
        ...
        if stmt.indices:
            if not var_info.is_array:
                ...
                for i, idx in enumerate(stmt.indices):
                    idx_type = self.infer_expression_type(idx)
                    if idx_type != TypeKind.INTEGER and idx_type != TypeKind.UNKNOWN:
                        ...
                        self.errors.append(error)
                        return
        expr_type = self.infer_expression_type(stmt.value)
        if expr_type != TypeKind.UNKNOWN and var_info.type_kind != TypeKind.UNKNOWN:
            if not self.are_compatible(var_info.type_kind, expr_type):
                warning_msg = self.format_warning(
    ...
    def analyze_do_loop(self, stmt: DoLoop):
        ...
        if end_type != TypeKind.INTEGER:
            ...
            self.errors.append(error_msg)
        if stmt.step:
            step_type = self.infer_expression_type(stmt.step)
    ...
    def analyze_labeled_do_loop(self, stmt: LabeledDoLoop):
        if stmt.var not in self.symbol_table:
            error_msg = self.format_error(
                f"Переменная цикла '{stmt.var}' не объявлена",
                node=stmt,
    ...
    def analyze_do_while(self, stmt):
    ...
    def analyze_simple_if_statement(self, stmt: SimpleIfStatement):
        cond_type = self.infer_expression_type(stmt.condition)
        if cond_type != TypeKind.LOGICAL and cond_type != TypeKind.UNKNOWN:
            error_msg = self.format_error(
            ...
            self.errors.append(error_msg)
        self.analyze_statements([stmt.statement])
    ...
    def analyze_if_statement(self, stmt: IfStatement):
        ...
        for i, (elif_cond, elif_body) in enumerate(stmt.elif_parts, 1):
            elif_type = self.infer_expression_type(elif_cond)
            if elif_type != TypeKind.LOGICAL and elif_type != TypeKind.UNKNOWN:
                ...
                self.errors.append(error_msg)
            self.analyze_statements(elif_body)
        if stmt.else_body:
            self.analyze_statements(stmt.else_body)

    def analyze_arithmetic_if_statement(self, stmt: ArithmeticIfStatement):
        cond_type = self.infer_expression_type(stmt.condition)
    ...
    def analyze_print_statement(self, stmt: PrintStatement):
        for item in stmt.items:
            self.infer_expression_type(item)

    def analyze_io_statement(self, stmt):
        if isinstance(stmt, ReadStatement):
            for item in stmt.items:
                if item not in self.symbol_table:
                    if not self.implicit_none:
                        implicit_type, type_size = self.get_implicit_type(
                            item)
                        if implicit_type != TypeKind.UNKNOWN:
                            var_info = VariableInfo(
                                name=item,
    ...
    def analyze_call_statement(self, stmt: CallStatement):
        func_name = stmt.name
    ...
    def infer_expression_type(self, expr: Expression) -> TypeKind:
        if isinstance(expr, IntegerLiteral):
            ...
            return self.infer_binop_type(expr)
        elif isinstance(expr, UnaryOp):
            return self.infer_unaryop_type(expr)
        elif isinstance(expr, FunctionCall):
            return self.infer_function_call_type(expr)
        return TypeKind.UNKNOWN

    def infer_binop_type(self, expr: BinaryOp) -> TypeKind:
    ...
    def infer_unaryop_type(self, expr: UnaryOp) -> TypeKind:
    ...
    def infer_function_call_type(self, expr: FunctionCall) -> TypeKind:
        func_name = expr.name
        ...
        if func_name in self.builtin_functions:
            ...
            if arg_types and expr.args:
                for i, arg in enumerate(expr.args):
                    ...
                    if arg_type != TypeKind.UNKNOWN:
                        if not any(self.are_compatible(expected_type, arg_type) for expected_type in arg_types):
            ...
            if ret_type is None:
                if expr.args:
                    return self.infer_expression_type(expr.args[0])
            return ret_type if ret_type else TypeKind.UNKNOWN
        else:
    ...
    def are_compatible(self, target: TypeKind, source: TypeKind) -> bool:
    ...
    def are_comparable(self, left: TypeKind, right: TypeKind) -> bool:
    ...
    def analyze_dimension_statement(self, stmt: DimensionStatement):
        for name, dim_ranges in stmt.names:
            resolved_dim_ranges = self.resolve_dimension_ranges(dim_ranges, stmt, name)
            if len(dim_ranges) > 7:
                error_msg = self.format_error(
            ...
            if name in self.symbol_table:
                ...
                if not self.implicit_none:
                    implicit_type, type_size = self.get_implicit_type(name)
                    if implicit_type != TypeKind.UNKNOWN:
                        var_info = VariableInfo(
                            name=name,
                            type_kind=implicit_type,
    ...
    def is_constant_expression(self, expr: Expression) -> bool:
        return self.is_constant_expression_with_params(expr, set())

    def is_constant_expression_with_params(self, expr: Expression, declared_params) -> bool:
        if isinstance(expr, (IntegerLiteral, RealLiteral, LogicalLiteral, StringLiteral)):
            ...
            return (self.is_constant_expression_with_params(expr.left, declared_params) and
                    self.is_constant_expression_with_params(expr.right, declared_params))
        elif isinstance(expr, UnaryOp):
            return self.is_constant_expression_with_params(expr.operand, declared_params)
        elif isinstance(expr, FunctionCall):
            if expr.name in self.builtin_functions:
                return all(self.is_constant_expression_with_params(arg, declared_params) for arg in expr.args)
            return False
        elif isinstance(expr, ArrayRef):
            return False
        return False

    def evaluate_constant_expression(self, expr: Expression):
        return self.evaluate_constant_expression_with_params(expr, set())

    def evaluate_constant_expression_with_params(self, expr: Expression, declared_params: Dict[str, object]):
        if isinstance(expr, IntegerLiteral):
            return expr.value
        elif isinstance(expr, RealLiteral):
            return expr.value
            ...
            left_val = self.evaluate_constant_expression_with_params(
                expr.left, declared_params)
            right_val = self.evaluate_constant_expression_with_params(
            ...
            operand_val = self.evaluate_constant_expression_with_params(
    ...
    def analyze_parameter_statement(self, stmt: ParameterStatement):
        declared_in_this_stmt = {}
        for name, expr in stmt.params:
            declared_names = set(declared_in_this_stmt.keys())
            if not self.is_constant_expression_with_params(expr, declared_names):
                error_msg = self.format_error(
            ...
            if name in self.symbol_table:
                ...
                if var_info.is_parameter:
                    ...
                    value = self.evaluate_constant_expression_with_params(expr, declared_in_this_stmt)
                ...
                if expr_type == TypeKind.UNKNOWN:
                    if isinstance(expr, IntegerLiteral):
                        ...
                        self.errors.append(error_msg)
                        expr_type = TypeKind.INTEGER
                value = self.evaluate_constant_expression_with_params(
    ...
    def analyze_data_statement(self, stmt: DataStatement):
            ...
            for data_item, value in zip(vars_list, values):
                ...
                if indices:
                    ...
                    if var_info.is_array:
                        ...
                        self.errors.append(error_msg)
                        continue
                value_type = self.infer_expression_type(value)
                if value_type != TypeKind.UNKNOWN and var_info.type_kind != TypeKind.UNKNOWN:
                    if not self.are_compatible(var_info.type_kind, value_type):
                        warning_msg = self.format_warning(
    ...
    def analyze_subroutine(self, sub: 'Subroutine'):
        ...
        self.analyze_declarations(sub.declarations)
        ...
        for param in sub.params:
            ...
            if param_upper in self.symbol_table:
                ...
                if var_info.type_kind == TypeKind.UNKNOWN:
                    var_info.type_kind = self.get_implicit_type(param_upper)[0]

        self.analyze_statements(sub.statements)
    ...
    def analyze_function(self, func: 'FunctionDef'):
        ...
        self.analyze_declarations(func.declarations)
        ...
        if func_name_upper in self.symbol_table:
            ...
            if func_info.type_kind == TypeKind.UNKNOWN:
                func_info.type_kind = self.get_implicit_type(func_name_upper)[0]
        ...
        for param in func.params:
            ...
            if param_upper in self.symbol_table:
                ...
                if param_info.type_kind == TypeKind.UNKNOWN:
                    param_info.type_kind = self.get_implicit_type(param_upper)[0]

        self.analyze_statements(func.statements)
    ...
    def analyze_external_statement(self, stmt: 'ExternalStatement'):
    ...
    def analyze_common_statement(self, stmt: 'CommonStatement'):
        for block_name, variables in stmt.blocks:
            for var in variables:
                var_name = var.name if hasattr(var, 'name') else str(var)
                var_upper = var_name.upper()
                if var_upper not in self.symbol_table:
                    type_kind, type_size = self.get_implicit_type(var_upper)
                    var_info = VariableInfo(
                        name=var_upper,
                        type_kind=type_kind,
                        is_array=False,
    ...
    def analyze_goto_statement(self, stmt: 'GotoStatement'):
        pass

    def analyze_continue_statement(self, stmt: 'ContinueStatement'):
        pass

    def analyze_return_statement(self, stmt: 'ReturnStatement'):
        pass

    def analyze_stop_statement(self, stmt: 'StopStatement'):
        pass

    def analyze_exit_statement(self, stmt: 'ExitStatement'):
        if self.loop_nesting <= 0:
            error_msg = self.format_error(
                "Оператор EXIT можно использовать только внутри цикла",
                node=stmt,
                context="EXIT",
                suggestion="Переместите EXIT внутрь DO/DO WHILE или используйте GOTO"
            )
            self.errors.append(error_msg)

    def is_dimension_bound_constant(self, bound: object) -> bool:
        if isinstance(bound, int):
            return True
        return self.is_constant_expression(bound)

    def evaluate_dimension_bound(self, bound: object):
        if isinstance(bound, int):
            return bound
        return self.evaluate_constant_expression(bound)

    def resolve_dimension_ranges(self, dim_ranges: Optional[List[object]], node: ASTNode, name: str) -> Optional[List[Tuple[int, int]]]:
        ...
        for i, dim_spec in enumerate(dim_ranges):
            ...
            if not self.is_dimension_bound_constant(lower_bound) or not self.is_dimension_bound_constant(upper_bound):
                error_msg = self.format_error(
                    f"Размерности массива '{name}' должны быть константными",
                    node=node,
                    context=f"измерение {i+1}",
                    suggestion="Используйте целочисленные константы или PARAMETER-выражения"
                )
                self.errors.append(error_msg)
                return []
            lower_value = self.evaluate_dimension_bound(lower_bound)
            upper_value = self.evaluate_dimension_bound(upper_bound)
            if not isinstance(lower_value, int) or isinstance(lower_value, bool) or not isinstance(upper_value, int) or isinstance(upper_value, bool):
                error_msg = self.format_error(
                    f"Размерности массива '{name}' должны быть целочисленными",
                    node=node,
                    context=f"измерение {i+1}",
                    suggestion="Используйте целые константы или INTEGER PARAMETER"
                )
                self.errors.append(error_msg)
                return []
            if lower_value > upper_value:
                error_msg = self.format_error(
                    f"Неверный диапазон для измерения {i+1} массива '{name}'",
                    node=node,
                    context=f"начальное значение ({lower_value}) больше конечного ({upper_value})",
                    suggestion="Сделайте нижнюю границу меньше или равно верхней"
                )
                self.errors.append(error_msg)
                return []
